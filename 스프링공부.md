...
- ## IoC/DI  (생성과 제어)
    - ### IoC. Inversion of Control. 제어의 역전
        ...
        부모 오브젝트가 자신이 사용할 오브젝트를 스스로 선택하지 않는다. 당연히 생성하지도 않는다.
    ...
    커피가 아니라 아메리카노, 카페라떼, 민트초코 로 이 프로그래머가 먹을대상이 바뀌게되면 프로그래머의 소스가 바뀌어야된다.
      ...
      - 쉽게 설명1
        ...
        스프링은 일체형이 아니라 조립형으로 만든것이 큰 특징이라는 것이다.
...
## ★★★★화면에서 백엔드로 데이터 넘기기 (컨트롤러에서 데이터 받기)★★★★
  1. 주소창에 쿼리스트링으로 ?키=값&키=값 넣고 컨트롤러에서는 @RequestParam String 넘어온키1, @RequestParam String ("넘어온키2") String 백에서쓸변수명'
    예시) user?id="sirin"&name="한시린"

    @GetMapping("/getUser")
    public void userGET(@RequestParam ("name") String name){}
    // 요청 파라미터에 name이라는 이름으로 넘어온 값을 이 함수에서 name이라는 이름으로 쓰겠다
    // 이런 노가다 작업을 안하기 위해서 객체에 변수들을 묶어서 넣었고,
    // getName이라는 함수가 만들어져 있으면 이런 작업을 안해줘도 자동으로 매칭되도록 스프링이 구현해놨다.
...
### service 와 serviceImpl
  ...
  - 인터페이스에
    ...
    구현체에서는 modify의 실행(작동)방식이 변경 될 수 있다.
    그런데 인터페이스 없이 직접 그 인스턴스를 썼다면 서비스가 달라지면 컨트롤러도 달라져야 한다.
    하지만 인터페이스라는 매개체를 통해서 매개체를 통해서 구현체를 실행했기 때문에 본문(실행)방식이 달라져도 컨트롤러의 소스는 변함없다.
