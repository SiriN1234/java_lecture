...
## JSP가 처리되는 과정
1. 웹브라우저에 주소 입력(어디에 어떤 요청을 했는지가 담겨있다.)
...
##### 조금 어려운 jsp 만들기
  ...
  서블릿 컨테이너가 제공한다. 내장객체란말은 선언없이 이미 선언되어 있기에 바로 사용할수 있다는 말
      ...
      - ### session
          쿠키와 더불어 데이터를 유지할 수 있는 기간을 늘려주는 기능
  ...
  - ### 6. JSTL (JSP Standard Tag Library). 표준액션태그와 태그라이브러리
      ...
      - 원래라면 <% if (변수 > 200) %>
                ...
                <% else %>
                html 내용
        저런 귀찮음을 아래와 같이 html의 태그로 자바코드를 쉽게 쓸 수 있도록 문법을 지원해준다.
      - <c:if test="${변수>2000}">
              html내용
        </c:if>
...
# JSTL  (Jsp Standard Tag Library)
  ...
  - 사용하는 방법 : 1. 톰캣 공식 사이트에서 jstl라이브러리(jstl.jar)과 standard.jar 를 다운받아서 WEB-INF/lib 에 넣는다.
              ...
              3. 사용
               ...
               - 그냥 코드를 넣는 것과 <c:out>을 통해 코드를 넣는 것의 차이
                  : 
  - 종류 :
  ...
  - 예제4. 회원권한을 id옆에 붙여서 표시하는 방법. 그대로 응용하면 권한에 따라 메뉴 보이게 지정
    ...
    # EL (Expression Language. 표현언어)
      ...
      - 문법  **${표현식}**
        - 표현식 :  <%=add%>      표현언어 :  ${add}
      - 위의 식에서 표현식에서는 add를 자바변수명으로 인식하는데, 표현언어에서는 add라는 속성명으로 인식한다. 사실 엄밀하게 뜯어보면 내부적으로 자바빈의 getter 메소드가 호출된다.
