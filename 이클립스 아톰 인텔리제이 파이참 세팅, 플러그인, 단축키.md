...
----------------------------------------------------------------------------------------
# 이클립스, STS
- 일단 이클립스(STS)는 JDK를 통해서 컴파일을 하지 않음. 기본적으로 내장된 컴파일러를 사용한다. 그런데 런타임 환경은 사용자가 설치한(설정한) 것을 사용함.
- STS 4.14.1 버전은 JDK 17의 문법을 지원함. 같이 사용하는 IDE와 사용하는 JDK의 버전 호환 문제가 일어날 수 있다
