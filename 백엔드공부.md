...
URI : Uniform Resource Identifier. 통합자원식별자
  ...
  뜻 : https로 get 요청을 한다. www.fastcampu.co.kr이라는 도메인을 가진 서버에게 그 서버의 80 포트를 리스닝하고 있는 웹 서버에게 resources 하위의 saple 하위의 1번을 요청한다.
  이 요청이 들어오면 응답을 지정해 놓는다.
URL : Uniform Resource Locator
  인터넷상의 특정 리소스가 어디있는지 위치를 알려주기위한 규약
  예) https://www.fastcampus.co.kr/fastcampus.pdf
  뜻 : https로 get 요청을 한다. www.fastcampu.co.kr이라는 도메인을 가진 서버에게 그 서버의 80 포트를 리스닝하고 있는 웹서버의 루트폴더에 있는 fastcampus.pdf를 달라.
...
- Advice(부가기능) :  부가기능을 정의한 코드. 5가지 종류
    ...
    - (**유의**)Around를 쓰고 return에 proceed()한 오브젝트를 바로 리턴 해버리면
      실행되어야 할 aop가 실행 되지 않는 것에 유의
